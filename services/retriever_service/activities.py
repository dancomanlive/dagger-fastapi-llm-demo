...
EMBEDDING_MODEL_NAME = os.getenv("EMBEDDING_MODEL", "fast-bge-small-en")
...
async def search_documents_activity(query: str, collection_name: str, top_k: int = 5) -> Dict[str, Any]:
    ...
    try:
        ...
        # Perform text-based search using query_points, similar to main.py
        search_results = qdrant_client.query_points(
            collection_name=collection_name,
            query=qdrant_models.Document(
                text=query,
                model=EMBEDDING_MODEL_NAME  # Use the embedding model name from environment variable
            ),
            using=EMBEDDING_MODEL_NAME,  # Specify the vector name, consistent with main.py
            limit=top_k,
            with_payload=True
        )
        ...
        # Transform results
        retrieved_documents = []
        for hit in search_results.points: # Iterate over .points
            payload_text = None
            if hit.payload:
